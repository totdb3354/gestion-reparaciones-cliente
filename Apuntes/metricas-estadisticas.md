...
- **Leyenda con IMEIs** (ajuste 2026-09-04): cada chip de técnico muestra "`nombre` · `lleva`/`típico` IMEIs" — lo que tocó en el **último periodo visible** (hoy en Día, esta semana en Semana, este mes en Mes) contra el **típico por técnico** = media por técnico-periodo trabajado de IMEIs del rango de referencia (misma fórmula que el Promedio de puntos, sin excluidos; se muestra con un decimal — "12/12,4" — y la comparación del verde es exacta). Chip en **verde** al alcanzar el típico. Ojo: la línea naranja del gráfico es de PUNTOS — por eso la referencia de IMEIs va incrustada en el chip. Mismo criterio que el Agrupado por IMEI filtrado por técnico; un teléfono con varios trabajos cuenta 1; la cuenta la hace el servidor por periodo (campo aditivo `nImeis`; con servidor antiguo el sufijo no aparece). El detalle de *cuáles* son: popover → "Ver IMEIs". La vista/serie completa de IMEIs queda para la analítica web (F4).
