...
| Hardening | Backend sin puerto al host; SSH solo por clave; fail2ban leyendo logs de nginx por bind mount; rate limiting en `/api/auth/login`; cabeceras de seguridad; contraseña SQL y secreto JWT nuevos; usuario MariaDB dedicado; `include-message` de errores desactivado; backup diario | Recomendaciones de la guía Arsys y lecciones del incidente de fonestockroom, todas pendientes hoy |
