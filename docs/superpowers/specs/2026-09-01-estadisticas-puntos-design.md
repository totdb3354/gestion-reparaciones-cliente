...
- **Selector de métrica** (dos estados, tipo toggle): **Puntos** = total del periodo; **Puntos/día** = total ÷ días laborables (L–V) del periodo. Cambiarlo recalcula eje, series, promedio, tooltips y tarjetas. Todo en cliente, del mismo dato.
  ...
  - Granularidad Día: puntos/día del día = puntos (÷1); días no laborables con trabajo se muestran tal cual. Granularidad por defecto: **Día** (ajuste smoke 2026-09-01; antes Semana).
...
- **Promedio**: línea **discontinua horizontal** (paralela al eje X), naranja actual (`#C07800`), con etiqueta "Promedio". Valor = media por técnico-periodo **trabajado** (suma ÷ nº de técnico-periodos con actividad; **las ausencias no diluyen**) sobre el **rango de referencia**: el filtro de fechas si lo hay, o la última ventana estándar de la granularidad sin filtro. La vara es **FIJA durante la navegación con flechas** (retroceder compara contra el ritmo del rango de referencia, no contra cada tramo; para la vara de una época, se filtra esa época). El tooltip "Por encima/Por debajo" usa el mismo rango y su cabecera lo dice ("Promedio del equipo (30 días con actividad): …"). — Ajustes smoke 2026-09-03: antes los huecos contaban como 0 (todos salían "por encima") y la vara se recalculaba con cada clic de flecha. Se recalcula al cambiar granularidad, fechas, ventana o métrica. Es la "referencia" de hoy con nombre visible y en puntos; **siempre visible, sin checkbox propio** (es la vara de medir de la vista).
- **"Ocultar medias"** (ajuste smoke 2026-09-04; antes "Ver medias" off por defecto): las líneas x̄ discontinuas de media por serie **se ven por defecto** y el checkbox las oculta. Solo de las series visibles. Los checkboxes **"Todos" y "Actividad" desaparecen** (redundantes con la selección explícita).
- **Navegación temporal**: fuera el slider; **flechas ◀ ▶** que desplazan la ventana (misma ventana por granularidad: 30 días / 16 semanas / 12 meses / 5 años) con etiqueta del rango visible en medio ("16 sem · 18 may — 30 ago"). Flecha deshabilitada en el extremo. Al abrir, posicionado en lo más reciente. **El filtro de fechas Desde/Hasta acota el rango navegable**: la ventana se encoge para ajustarse a rangos cortos (se ven enteros) pero nunca crece más del estándar — un rango largo se recorre con las flechas dentro del filtro (ajustes smoke 2026-09-01/02). La barra de navegación está **siempre visible**: cuando todo cabe (o no hay datos) las dos flechas quedan deshabilitadas, y en un extremo se deshabilita la del lado correspondiente (ajuste smoke 2026-09-02). **AJUSTE 2026-09-04: las flechas se ELIMINAN** — lo mostrado es siempre el rango elegido (últimos 30 días con actividad por defecto; con filtro de fechas, el rango filtrado ENTERO — para rangos largos en Día, la lectura cómoda es subir la granularidad) y la etiqueta lo resume; el pasado se consulta con el filtro de fechas, con lo que las varas son siempre las del rango mostrado.
- **Huecos sin actividad** (vacaciones, ausencias): los periodos visibles en los que un técnico no trabajó se pintan a **0** en su serie (ajuste smoke 2026-09-02). Además de leerse honesto (ese día no hizo nada, igual que asumen la serie Equipo y el Promedio), corrige un bug real: sin el relleno, el eje X de categorías se desordenaba cuando una serie tenía huecos (las fechas de la vuelta de vacaciones se colaban en medio del eje).
- **RESUELTO en ronda 2** (`2026-09-02-estadisticas-puntos-ronda2-design.md` §4): **panel de exclusión de técnicos de las estadísticas** — la media se desinfla con perfiles que no reparan a jornada completa (logística entrando como supertécnico, supertécnicos con otras tareas). Diseño conversado: columna `ES_ESTADISTICA` en `Tecnico` (ALTER aditivo, default sí), exclusión TOTAL de la vista de Técnicos (Promedio, Equipo, tarjetas y desplegable; Historial/Agrupado/Log no se tocan), panel solo ADMIN junto a ⚙ Valores, y el excluido sigue viéndose a sí mismo en su propia vista. Complementa (no sustituye) al futuro rol LOGISTICA de F3. Camino (decisión usuario 2026-09-02): **entra en la 0.16.2** — la release espera a esta ronda 2 (junto con tarjetas, selector de métrica y "facilitar la asignación de glass", que es feature aparte de la línea hotfix), a preparar con spec+plan en una sesión nueva.
- **RESUELTO en ronda 2** (`2026-09-02-estadisticas-puntos-ronda2-design.md` §3): el selector de métrica **Puntos / Puntos·día** quedó aparcado para repasar (en granularidad Día ambas métricas coinciden y el toggle no aporta; candidatos: deshabilitarlo en Día, moverlo, o repensar su presentación).
- **RESUELTO en ronda 2** (`2026-09-02-estadisticas-puntos-ronda2-design.md` §2, formato objetivo): las tarjetas resumen resultaban confusas (el delta de "Puntos" compara mes-en-curso contra mes completo y sale un rojo enorme a principios de mes); replantearlas queda aparcado — candidatos: quitar el delta de "Puntos", comparar contra los mismos días transcurridos del mes anterior, o simplificar a una sola tarjeta.
...
- **Clic en un vértice** → **popover de desglose** anclado al punto: "Marcos — sem 18-24 ago: **14,5 pts** = 9 normales (11,0) + 3 glass (2,0) + 6 pulidos (1,5) · 2 sin piezas", con botones **"Ver IMEIs"** primero (ajuste smoke 2026-09-01/02: aterriza en el Agrupado por IMEI con las mismas fechas y técnico — se ven los IMEIs con al menos un trabajo suyo en el rango) y **"Ver en Historial"** después (la navegación actual, `setFiltroInicial` con fechas del periodo + técnico). Serie Equipo → desglose del equipo y navegación sin técnico. Promedio no clicable. En rol técnico solo su serie es clicable (regla actual).
...
5. Clic en vértice: popover con desglose que cuadra a mano con 1-2 reparaciones conocidas (normal con 2 piezas, glass+marco, pulido, sin piezas); "Ver en Historial" aterriza con filtros bien puestos y "Ver IMEIs" abre el Agrupado con las mismas fechas + técnico.
