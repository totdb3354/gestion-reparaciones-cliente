...
1. **Técnico del token y propiedad de la asignación** en `POST /api/reparaciones/completa`, `POST /api/reparaciones/{idAsignacion}/filas`, `POST /api/reparaciones/{idAsignacion}/agotar-componente` y `GET|PUT|DELETE /api/reparaciones/{idRep}/borrador`: el técnico efectivo es el del token; el `idTec` del cuerpo se **ignora** (no se rechaza: el JavaFX lo sigue enviando); la asignación debe ser de ese técnico o la respuesta es 403. La regla vive en un componente único junto a `FiltroTecnico`, con tests propios. Si la lectura del DAO demuestra que hoy un supertécnico completa asignaciones de otro desde algún flujo real del JavaFX, ese caso se conserva y se documenta en el plan; si no, aplica la misma regla.
...
3. **Roles**: `/api/solicitudes-stock` — crear: TECNICO y SUPERTECNICO; listar, contar, cambiar estado y borrar: SUPERTECNICO, con lectura para ADMIN como fija la spec maestra. `PATCH /api/componentes/{id}/stock`: SUPERTECNICO, tras verificar en el hotfix quién lo llama para no romper a la tienda (si lo usa un TECNICO en un flujo real, se mantiene ese rol y se anota).
