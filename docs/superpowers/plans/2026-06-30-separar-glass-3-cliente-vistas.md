...
- **Task 5 — Exports/contadores:** ✅ HECHA (compila + 113 tests). `AgrupadoController.exportarCSV` (maestro con contadores Rep/Glass/Pul + detalle cronológico con Tipo); los 3 roles delegan cuando el panel Agrupado está visible; Historial de Glass exporta con nombre propio (`historial_glass`/`mis_glass`). Contadores/badges ya incluían glass (Plan 2).
- **Cleanup del código maestro/detalle muerto:** ⬜ PENDIENTE. **No es "borrar métodos sin uso":** el subsistema (mostrarDetalle, buildTablaItems, resetarModo, volverAGrupos, adaptarFiltrosMaestro, abrirDialogoObservacionTelefono, crearBarraNavegacion, ramas MAESTRO/DETALLE de aplicarFiltros, ramas `instanceof GrupoImei` de las cell/row factories, menú editarObs/Cli, columnas obs.teléfono/cliente/revisión) sigue cruzándose entre sí, así que hay que retirarlo **entero y coherente** por controlador (~500 l/rol) + quitar columnas del FXML. Toca el Historial recién validado → requiere su propio smoke. Mejor en una pasada dedicada.
