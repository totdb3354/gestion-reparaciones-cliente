...
- [x] **Step 2: Copiar el cálculo y su test desde el cliente, sin editarlos todavía**
...
- [x] **Step 4: Adaptar los dos ficheros copiados**
...
- [x] **Step 5: Ejecutar los tests portados y verificar que pasan**
...
- [x] **Step 1: Escribir el test de la zona horaria, que debe fallar**
...
- [x] **Step 3: Añadir `diaDeHoy()` a `CargaTecnicos`**
...
- [x] **Step 5: Crear los records de la respuesta**
...
- [x] **Step 6: Escribir el test del endpoint, que debe fallar**
...
- [x] **Step 7: Ejecutar y verificar que falla**
...
- [x] **Step 9: Ejecutar y verificar que pasan**
...
- [x] **Step 10: Ejecutar la suite entera del servidor**
...
- [x] **Step 2: Levantar el servidor de la Task 2 en local y regenerar el contrato**
...
- [x] **Step 4: Comprobar que compila**
...
- [x] **Step 1: Escribir el test de `badgesEstado`, que falla**
...
- [x] **Step 3: Implementar `badgesEstado` y las columnas**
...
- [x] **Step 3: Implementar la página mínima**
...
- [x] **Step 4: Sustituir el placeholder en el router**
...
- [x] **Step 5: Ejecutar los tests y la comprobación completa**
...
- [x] **Step 1: Escribir el test del hook, que falla**
...
- [x] **Step 4: Escribir el test del menú, que falla**
...
- [x] **Step 6: Implementar el menú y cablearlo**
...
- [x] **Step 3: Implementar y cablear**
...
- [x] **Step 4: Ejecutar la suite completa de la web**
...
- [x] **Step 2: Marcarla contra las capturas**
...
- [x] **Step 3: Verificación completa de los tres repos**
...
- [x] **Step 4: Smoke contra producción**
...
- [x] **Step 5: Capturas de la web y commit de cierre**
...
- [x] **Step 6: Parar y pedir OK al usuario**
...
**Entregado el 2026-09-22.** Servidor `main` `65b660c` y web `main` `6907408` (tag `v0.4.0`), desplegados en producción con el servidor antes que la web. Suites: servidor 231, web 867, cliente JavaFX sin cambios. Smoke e2e 5/5 contra producción. El contrato publicado es idéntico al que consume la web. Ficha de paridad `docs/paridad/asignaciones.md` con todas sus casillas marcadas.

Las dieciocho tareas se ejecutaron con un implementador y una revisión por tarea, y una revisión final de toda la rama. Durante la ejecución, el código existente y el contrato real mandaron sobre el código de ejemplo de este plan, tal como prevé su autorrevisión. Los ajustes:

- **Reutilización en vez de piezas nuevas.** El orden de prioridad (`ordenarPendientes`), los predicados de filtro, el centinela de cliente, `tipoDe`, `badgesEstado`, `BadgeTipo`, `CeldaImeiPendiente`, `SelectorLista` y las mutaciones del teléfono ya existían y estaban probados, así que se compusieron en lugar de recrearlos. Por eso no existen `orden.ts` ni `EditorModelo.tsx`.
- **El filtro Estado combina con O**, como el JavaFX y la spec §9, no en cascada.
- **Escrituras según el contrato real**: el pulido escribe por `/api/pulidos/...`; reasignar y editar comentario comparten el mismo PATCH con comentario y `updatedAt`; el borrado tiene tres ramas (pulido, incidencia, asignación); el cuerpo de técnicos de glass es `{ habilitado }`.
- **Deshacer robusto**: relee la fila fresca antes de revertir, distingue lo confirmado por el servidor de lo pintado en optimista y no pisa el cambio de otro usuario. La reasignación se pinta al instante.
- **Refresco congelado (D4) completo**: también ante el foco de la ventana, con `ComboNavy` y `MultiSelect` avisando de apertura y cierre de forma simétrica.
- **Carga de técnicos**: la lista de entrada es la unificada (reparación, glass y pulido), como en el JavaFX; el día se resuelve en Europe/Madrid con reloj inyectable.
- **Añadidos por paridad** detectados al marcar la ficha y comparar capturas: el contador "N asignados" bajo el IMEI, la tabla a todo el ancho con el identificador entero, el contador del lateral con el total y el aviso de urgente nombrando el IMEI.
- **La entrada por defecto del supertécnico sigue en Historial**, como define el FXML para cada rol.

Pendiente para el sub-proyecto 3b: el modal "Asignar trabajos" y la predicción de glass (tag `v0.5.0`).
