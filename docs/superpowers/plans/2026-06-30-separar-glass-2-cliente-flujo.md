...
- **Task 4 — Modal con selector:** ✅ HECHA (compila + 109 tests; pendiente smoke). **4a**: selector `Reparación | Glass` + tipo por entrada + dedup `(IMEI,tipo)` + guardado A/AG; dedup por categoría sin servidor (greying desde `datos` + guard `existeAsignacionParaTecnico(...,categoria)`). **4b**: selector a 3-way (+ `Pulido`), sub-pila ligera de pulido vía helper `construirPulidoPane` (técnico/comentario por defecto + escaneo/pegado + lista editable por fila), `StackPane` que conmuta pila rica ↔ pulido, un único Guardar confirma A/AG + AP (guard pulido con `?tipo=P`), clase `toggle-pill-mid` nueva en `app.css`.
- **Task 5 — Pendientes 3 pestañas:** ✅ HECHA (compila + 109 tests; pendiente smoke). `PendientesTecnicoController` gana modo glass (`setModoGlass` → lista `AG` del técnico, completa reutilizando el modal que se filtra a glass por el prefijo `AG` y genera `G` en servidor, borra incidencia con `?tipo=G`). Toggle de Pendientes a 3-way `Reparaciones|Glass|Pulido` en **técnico y supertécnico** (2º `fx:include` de `PendientesTecnicoView` en modo glass). Badge "Pendientes" suma rep+glass; recargar/mostrarPanel/export CSV contemplan las 3. Admin no tiene Pendientes (no completa).

**Plan 2 (cliente flujo) COMPLETO** salvo smoke en preproducción. Sigue Plan 3 (cliente vistas): Historial 3-way plano + apartado "Agrupado" compartido + filtro de logs `TIPOS_ACCION`.
